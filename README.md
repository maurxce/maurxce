# 💻 Welcome to my Github Profile!

My name is Maurice and I'm a 22 year old developer from Germany 🇩🇪.  
Looking for some of my work? Check out my Pins and Organizatins! 🙂

## ⚡ Currently Using

**☕ Java**  
![IDEA, Java, Spring, MYSQL](./images/java.svg "IDEA, Java, Spring, MYSQL")  

**🌐 Web**  
![VS Code, TypeScript, Astro, React, MongoDB](./images/web.svg "VS Code, TypeScript, Astro, React, MongoDB")  

**🛠️ Tools**  
![Proxmox, Ubuntu, Windows, Docker](./images/tools.svg "Proxmox, Ubuntu, Windows, Docker")  

## 📲 Contact me

[![Discord](./images/discord.svg "Discord")](https://discordapp.com/users/398509167351955456/)
[![Email](./images/gmail.svg "Email")](mailto:maurice@schorn.md)



